...
int main(int argc, char **argv)
{
   ...
   runManager->SetUserInitialization(new E6DetectorConstruction());

   // Physics list
   //auto physicsList = new E6PhysicsList();
   auto physicsList = new QGSP_BERT_HP();
   runManager->SetUserInitialization(physicsList);

   // Primary generator action and User action intialization
   runManager->SetUserInitialization(new E6ActionInitialization());
